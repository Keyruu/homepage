...
function clearArrowheadTimeout() {
  if (arrowheadTimeout) {
...
function updatePath() {
  const path = document.querySelector("path.toc-marker");
  const arrowhead = document.querySelector(".toc-arrowhead");
  ...
  const angle =
    (Math.atan2(endPoint.y - beforeEndPoint.y, endPoint.x - beforeEndPoint.x) *
      180) /
    Math.PI;
  arrowhead.setAttribute(
    "transform",
    `translate(${endPoint.x - 5}, ${endPoint.y - 5}) rotate(${angle} 5 5)`,
  );
